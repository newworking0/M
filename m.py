...
# Maximum allowed attack time
MAX_ATTACK_DURATION = 180  # Default max attack time in seconds
...
def set_max_duration(message):
    ...
    if user_id in admin_id:
        command = message.text.split()
        if len(command) == 1:
            response = "⚠️ Usage: /setmaxduration <seconds>\nExample: /setmaxduration 120"
        else:
            try:
                new_duration = int(command[1])
                if new_duration <= 0 or new_duration > 3600:
                    response = "❌ Duration must be between 1 and 3600 seconds."
                else:
                    global MAX_ATTACK_DURATION
                    MAX_ATTACK_DURATION = new_duration
                    response = f"✅ Max attack duration has been set to {new_duration} seconds."
            except ValueError:
...
💥 /setmaxduration <sec> : Mᴀx Aᴛᴛᴀᴄᴋ Dᴜʀᴀᴛɪᴏɴ Sᴇᴛ Kᴀʀɴᴀ.
'''
    bot.reply_to(message, response)
